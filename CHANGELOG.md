...
- **DB cold-start audit + idempotent bootstrap** — Migration history collapsed from 12 phase-named files (which couldn't run on a fresh DB — first migration assumed a baseline schema applied via `drizzle-kit push` during pre-migration phases) to a single `0000_initial.sql` baseline generated from `schema.ts`. All 20 CHECK constraints added to `schema.ts` so it's the real source of truth. Fixed latent bug: `arena_llm_calls.call_type` constraint now includes `verdict_synthesis` (the code emitted it but the prior constraint would have rejected it). New top-level `pnpm db:bootstrap` runs build + migrate + seed; idempotent on re-run.
